...
hold on;
matlab_plot = plot(tout, rocket_damping_ratio, 'm--*');
openrocket_plot = plot(openrocket_VarName1(1:indexmax_openrocket), openrocket_DampingCoefficient(1:indexmax_openrocket),'b--.');
rocksim_plot = plot(Rocksim_Time(1:indexmax_Rocksim), Rocksim_DampingRatio(1:indexmax_Rocksim),'k- .');
hold off;
ylim([0,1]);
